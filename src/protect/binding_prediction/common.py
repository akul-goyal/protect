...
from protect.common import get_files_from_filestore, untargz, export_results
...
def spawn_antigen_predictors(job, transgened_files, phlat_files, univ_options, mhc_options):
    ...
    for allele in mhci_alleles:
        for peplen in ['9', '10']:
            ...
            mhci_job = job.addChildJobFn(predict_mhci_binding, pept_files['T_' + peptfile], allele,
                                         ...
                                         memory='100M', cores=1)
            mhci_preds[(allele, peplen)] = mhci_job.addChildJobFn(predict_normal_binding,
                                                                  mhci_job.rv(),
                                                                  {x: y for x, y in pept_files if
                                                                   peplen in x}, allele, peplen,
                                                                  univ_options, mhci_options,
    ...
    for allele in mhcii_alleles:
        ...
        mhcii_job = job.addChildJobFn(predict_mhcii_binding, pept_files['T_15_mer.faa'],  allele,
                                      ...
                                      cores=1)
        mhcii_preds[(allele, 15)] = mhcii_job.addChildJobFn(predict_normal_binding, mhcii_job.rv(),
                                                            {x: y for x, y in pept_files if
                                                             '15' in x}, allele, 15, univ_options,
                                                            mhcii_options, disk='100M',
...
def _process_consensus_mhcii(mhc_file):
    """
...
def _process_sturniolo_mhcii(mhc_file):
    """
...
def _process_net_mhcii(mhc_file):
    """
...
def _get_normal_peptides(mhc_df, iars, peplen):
    ...
    :rtype: tuple(list, pandas.DataFrame)
    """
    peplen = int(peplen)
    normal_peptides = []
...
def merge_mhc_peptide_calls(job, antigen_predictions, transgened_files, univ_options):
    ...
    with open('/'.join([work_dir, 'mhci_merged_files.list']), 'w') as mhci_resfile:
        for key in mhci_preds:
            tumor_df = pandas.read_json(job.fileStore.readGlobalFile(mhci_preds[key]['tumor']))
    ...
    with open('/'.join([work_dir, 'mhcii_merged_files.list']), 'w') as \
        ...
        for key in mhcii_preds:
            tumor_df = pandas.read_json(job.fileStore.readGlobalFile(mhcii_preds[key]['tumor']))
            # TODO: There must be a better way of doing this
            if mhcii_preds[key]['predictor'] == 'Consensus':
                normal_df = _process_consensus_mhcii(
                    job.fileStore.readGlobalFile(mhcii_preds[key]['normal']))
...
def print_mhc_peptide(neoepitope_info, peptides, pepmap, outfile, netmhc=False):
    ...
    """
    if netmhc:
        allele, pept, pred, core, pname = neoepitope_info
        peptide_names = [pname]
    else:
    ...
    for peptide_name in peptide_names:
        print('{ni.allele}\t{ni.pept}\t{pname}\t{ni.core}\t0\t{ni.pred}\t{ni.normal_pred}'
